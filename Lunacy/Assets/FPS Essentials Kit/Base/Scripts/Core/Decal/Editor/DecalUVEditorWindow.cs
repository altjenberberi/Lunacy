...
public sealed class DecalUVEditorWindow : EditorWindow
{
    static public void Initialize (Decal d)
    {
        DecalUVEditorWindow window = (DecalUVEditorWindow)GetWindow(typeof(DecalUVEditorWindow), false, "Decal UV");
    ...
    private void OnGUI ()
    {
        ...
        if (currentDecal != null)
        {
            if (texture != null)
            {
                ...
                GUILayout.Space(4);
                zoom = GUILayout.HorizontalSlider(zoom, 0.1f, 10f, GUILayout.Width(Screen.width * 0.7f));
                GUILayout.EndVertical();
                GUILayout.FlexibleSpace();
                GUILayout.Label(zoom.ToString("F1"));
                GUILayout.EndHorizontal();
                GUILayout.EndArea();

                GUILayout.BeginArea(new Rect(5, 30, Screen.width - 10, 100));
                GUILayout.BeginHorizontal();
